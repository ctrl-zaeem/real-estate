...
def inbox(request):
    ...
    messages_received = Messages.objects.filter(receiver_id=user).select_related('sender', 'property')
    messages_sent = Messages.objects.filter(sender_id=user).select_related('receiver', 'property')
...
def send_message(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message.sender = get_object_or_404(Person, user_id=request.session['person_id'])
            message.save()
...
def inbox_view(request):
    current_user_id = request.session.get('person_id')  # Get current user from session
    if current_user_id is None:
        return redirect('login')

    messages = Messages.objects.filter(receiver__user_id=current_user_id).order_by('-sent_date')
    return render(request, 'home/inbox.html', {'messages': messages})
